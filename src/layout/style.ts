...
const Div: any = Styled.div`
    ...
    header{
        ...
        ${({ darkMode }: any) => (darkMode ? `background: #272B41;` : '')};
        z-index: 999;
    ...
    .ant-layout-sider{
            ...
            .ant-menu{
                ...
                .ant-menu-item{
                    ...
                    &.ant-menu-item-active{
                        border-radius: 4px;
                        ${({ darkMode }: any) => (darkMode ? `background-color: rgba(255, 255, 255, .05);` : '')};
                    }
                    a{
                        width: 100%;
        ...
        .sidebar-nav-title{
            ...
            ${({ darkMode }: any) => (darkMode ? `color: rgba(255, 255, 255, .38);` : 'color: #9299B8;')};
...
const SmallScreenAuthInfo: any = Styled.div`
        ${({ darkMode }: any) => (darkMode ? `background: #272B41;` : 'background: #fff')};
        width: 100%;
        position: fixed;
        margin-top: ${({ hide }: any) => (hide ? '0px' : '64px')};
        top: 0;
        ${({ theme }) => (!theme.rtl ? 'left' : 'right')}: 0;
        transition: .3s;
        opacity: ${({ hide }: any) => (hide ? 0 : 1)};
        z-index: ${({ hide }: any) => (hide ? -1 : 1)};
        box-shadow: 0 2px 30px #9299b810;

`;

const SmallScreenSearch: any = Styled.div`
        ${({ darkMode }: any) => (darkMode ? `background: #272B41` : 'background: #fff')};
        width: 100%;
        position: fixed;
        margin-top: ${({ hide }: any) => (hide ? '0px' : '64px')};
